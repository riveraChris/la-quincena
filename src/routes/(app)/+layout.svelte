...
{#if data.session}
	<div class="flex h-[100dvh] flex-col bg-surface text-text-primary">
		...
		<!-- Bottom Nav -->
		<nav class="border-t border-nav-border bg-nav pb-[env(safe-area-inset-bottom)]">
			<div class="mx-auto flex max-w-lg">
				{#each navItems as item}
					<button
						onclick={() => goto(item.href)}
						class="flex flex-1 flex-col items-center gap-1 py-2.5 text-[0.65rem] font-medium transition-colors
							{isActive(item.href) ? 'text-text-accent' : 'text-text-muted active:text-text-secondary'}"
					>
